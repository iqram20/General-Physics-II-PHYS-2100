<div align="center">

# PHYS 2100 — Formula Sheet  
## OpenStax *College Physics 2e* (Chapters 18–31)

</div>



# Fundamental Constants (SI Units)

- Electron charge: e = 1.602 × 10⁻¹⁹ C  
- Coulomb constant: k = 8.99 × 10⁹ N·m²/C²  
- Vacuum permittivity: ε₀ = 8.85 × 10⁻¹² C²/(N·m²)  
- Vacuum permeability: μ₀ = 4π × 10⁻⁷ T·m/A  
- Speed of light: c = 3.00 × 10⁸ m/s  


# Common Symbols & Parameter Definitions (SI Units)

- q, q₁, q₂ : electric charge (C)  
- Q : total charge (C)  
- Q_enc : enclosed charge (C)  
- n : number of elementary charges (unitless)  
- r : distance from charge, wire, or axis (m)  
- R : radius of sphere, cylinder, or curvature (m)  
- d : separation distance (m)  
- A : area (m²)  
- V : electric potential / voltage (V)  
- L : length of wire, conductor, or plate (m)  
- λ : linear charge density (C/m)  
- σ : surface charge density (C/m²)  
- ρ : volume charge density (C/m³)  
- E : electric field (N/C or V/m)  
- F : electric force (N)  
- θ : angle between vectors (degrees or radians)  
- m : mass (kg)  
- p : electric dipole moment (C·m)  
- Φ_E : electric flux (N·m²/C)  
- I : electric current (A)  
- R_eq : equivalent resistance (Ω)  
- R_int : internal resistance (Ω)  
- C : capacitance (F)  
- U : electric potential energy (J)  
- P : electric power (W)  
- v : particle speed or drift velocity (m/s)  
- a : acceleration (m/s²)  
- B : magnetic field (T)  
- Φ_B : magnetic flux (Wb = T·m²)  
- emf : electromotive force (V)  
- f : frequency (Hz)  
- T : period (s)  
- X_L : inductive reactance (Ω)  
- X_C : capacitive reactance (Ω)  
- Z : impedance (Ω)  
- n_opt : index of refraction (unitless)  
- f_lens : focal length (m)  
- d_o : object distance (m)  
- d_i : image distance (m)  
- m_mag : magnification (unitless)  
- h : Planck’s constant (J·s)  
- λ_wave : wavelength (m)  
- KE_max : maximum kinetic energy (J)  
- Φ_work : work function (J)  
- N : number of nuclei (unitless)  
- N₀ : initial number of nuclei (unitless)  
- T_½ : half-life (s)



# Chapter 18 — Static Electricity

- Charge:
  Q = n e  

- Coulomb’s Law:
  F = k |q₁ q₂| / r²  



# Chapter 19 — Electric Fields and Potential

- Electric field definition:
  E = F / q  

- Electric field of point charge:
  E = k q / r²  

- Electric force in field:
  F = q E  

- Electric potential:
  V = U / q  

- Potential energy:
  U = q V  



# Chapter 20 — Electric Potential and Capacitance

- Capacitance:
  C = Q / V  

- Parallel-plate capacitor:
  C = ε₀ A / d  

- Energy stored in capacitor:
  U = ½ C V²  



# Chapter 21 — Circuits, Bioelectricity, DC Instruments

- Current:
  I = ΔQ / Δt  

- Ohm’s Law:
  V = I R  

- Electric power:
  P = I V = I² R = V² / R  

- Series resistance:
  R_eq = R₁ + R₂ + ...  

- Parallel resistance:
  1/R_eq = 1/R₁ + 1/R₂ + ...  

- Terminal voltage:
  V = ε − I r  



# Chapter 22 — Magnetism

- Magnetic force on charge:
  F = q v B sinθ  

- Magnetic force on wire:
  F = I L B sinθ  

- Magnetic field (long straight wire):
  B = μ₀ I / (2π r)  



# Chapter 23 — Electromagnetic Induction

- Magnetic flux:
  Φ = B A cosθ  

- Faraday’s Law:
  emf = − ΔΦ / Δt  

- Motional emf:
  emf = B L v  



# Chapter 24 — Inductance, AC Circuits

- Inductance:
  V = L (ΔI / Δt)  

- Energy in inductor:
  U = ½ L I²  

- RMS voltage:
  V_rms = V_max / √2  

- RMS current:
  I_rms = I_max / √2  



# Chapter 25 — AC Circuits & Resonance

- Capacitive reactance:
  X_C = 1 / (2π f C)  

- Inductive reactance:
  X_L = 2π f L  

- Impedance:
  Z = √(R² + (X_L − X_C)²)  



# Chapter 26 — Electromagnetic Waves

- Wave speed:
  c = f λ  

- Energy of EM wave:
  E ∝ E² + B²  



# Chapter 27 — Geometric Optics

- Index of refraction:
  n = c / v  

- Snell’s Law:
  n₁ sinθ₁ = n₂ sinθ₂  

- Thin lens equation:
  1/f = 1/d_o + 1/d_i  

- Magnification:
  m = − d_i / d_o  



# Chapter 28 — Optical Instruments

- Angular magnification (magnifier):
  M = 25 cm / f  

- Telescope magnification:
  M = f_objective / f_eyepiece  



# Chapter 29 — Quantum Physics

- Photon energy:
  E = h f  

- Photoelectric equation:
  KE_max = h f − Φ  



# Chapter 30 — Atomic Physics

- Bohr radius:
  r_n = n² r₁  

- Energy levels:
  E_n = −13.6 eV / n²  



# Chapter 31 — Nuclear Physics

- Mass–energy equivalence:
  E = m c²  

- Half-life:
  N = N₀ (½)^(t / T₁/₂)  

